...
const sourcePath = path.join(__dirname, '../client/src/assets/images');
// Directory to save the optimized images
const outputPath = path.join(__dirname, '../client/src/assets/optimized/images');
...
// List of contact page images to be optimized
const contactImages = [
  'contact-hero-bg.jpg',
...
async function processAllImages() {
  try {
    ...
    const promises = contactImages.map(filename => optimizeImage(filename));
    
    // Wait for all optimizations to complete
    await Promise.all(promises);
    
    console.log('All contact images have been optimized successfully!');
