...
export default function ContactUsPage() {
  ...
  useEffect(() => {
    ...
    if (user) {
      ...
      // Safely access profile with type checking
      const userProfile = (user as any).profile;
      
      if (user.userType === 'jobseeker' && userProfile && 'firstName' in userProfile) {
        form.setValue('name', `${userProfile.firstName} ${userProfile.lastName}`);
        
        // If phone number is available, set it
        if ('phoneNumber' in userProfile) {
          form.setValue('phone', userProfile.phoneNumber);
        }
      } else if (user.userType === 'employer' && userProfile && 'companyName' in userProfile) {
        form.setValue('name', userProfile.companyName);
  ...
  const onSubmit = async (data: ContactFormValues) => {
    ...
    try {
      ...
      // Prepare the inquiry data with correct field names to match server expectations
      const inquiryData = {
        name: data.name,
        email: data.email,
        phone: data.phone,
        company: user?.userType === "employer" ? ((user as any).profile?.companyName || null) : null,
        message: data.message.replace(/'/g, "''"), // Escape single quotes for SQL safety
        inquiry_type: inquiryType, // Changed from inquiryType to inquiry_type to match DB schema
        ...
        >
          <div className="relative">
            ...
            <div className="flex justify-center gap-5 mt-6 relative z-10">
              ...
              <motion.a 
                href="https://x.com/xprtrecruitment" 
                target="_blank" 
                rel="noopener noreferrer"
                variants={{
                  hidden: { opacity: 0, scale: 0.8 },
