...
export async function registerRoutes(app: Express): Promise<Server> {
  ...
  app.post("/api/admin/impersonate", async (req, res) => {
    try {
      ...
      // Get admin profile to check if super_admin
      const adminProfile = await storage.getAdminByUserId(user.id);
      
      if (!adminProfile || adminProfile.role !== "super_admin") {
        return res.status(403).json({ 
          message: "Access denied. This feature is restricted to super administrators." 
        });
      }
      
      // Get the target admin ID to impersonate
      ...
      // Verify the target is a valid admin
      const targetAdmin = await storage.getAdmin(targetAdminId);
      
      if (!targetAdmin) {
        return res.status(404).json({ message: "Target admin not found" });
      }
      
      // Check that the target user is also an admin
      const targetUser = await storage.getUser(targetAdmin.userId);
      
      if (!targetUser || targetUser.userType !== "admin") {
        return res.status(400).json({ message: "Target user is not an admin" });
      }
      
      // Store the target admin's user ID in the session for impersonation
      req.session.impersonatedUserId = targetUser.id;
      
      // Return success response
      return res.status(200).json({ 
        message: "Admin impersonation started successfully",
        targetAdmin: {
          id: targetAdmin.id,
      ...
      res.status(500).json({ message: "Failed to start admin impersonation" });
    }
  });
  ...
  app.post("/api/admin/end-impersonation", async (req, res) => {
    try {
      ...
      // Return success response
      return res.status(200).json({ 
        message: "Admin impersonation ended successfully"
      });
      
    } catch (error) {
      console.error("Error ending admin impersonation:", error);
      res.status(500).json({ message: "Failed to end admin impersonation" });
    }
  });
