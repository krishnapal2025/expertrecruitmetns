...
export async function registerRoutes(app: Express): Promise<Server> {
  ...
  app.delete("/api/users/:id", async (req, res) => {
    try {
      ...
      if (userToDelete.userType === 'super_admin' && user.userType !== 'super_admin') {
        console.log("Non-super admin attempting to delete a super_admin account");
        return res.status(403).json({ 
          success: false,
      ...
      // We always expect a user ID from the frontend
      let idToDelete = userId;
      
      console.log(`Calling storage.deleteUser for user ID ${idToDelete} with type ${userType}`);
      try {
  ...
  // Removed duplicate endpoint - Super admin deletion endpoint is already defined at line ~3410
